...
const CreatePost = () => {
  ...
  const generateImage = async () => {
    if (form.prompt) {
      try {
        ...
        const response = await fetch("https://text-to-image13.p.rapidapi.com", {
          method: "POST",
          headers: {
            "x-rapidapi-key":
  ...
  const sendForm = async (dto) => {
    try {
      ...
      const res = await fetch(
        cfg.apiLink +"/api/v1/post/save-post",
        {
          method: "POST",
          mode:'cors',
          body: formData,
  ...
  const handleSubmit = (e) => {
    ...
    if (isFormValid) {
      ...
      if (!form.name) {
        alert("You need write your name");
      }
      if (!form.prompt) {
        alert("You must write text for generate picture");
      }
      if (!form.photo) {
        alert("You must generate photo to share it to community");
      }
    }
  };
    ...
    <section className="max-w-7xl mx-auto">
      <div>
        ...
        <p className="mt-2 text-[#666e75] text-[16px] max-w-[500px]">
          Create imaginative and visually stunning images throug DALL-E AI and
          ...
          <div className="relative bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 w-64 p-3 h-64 flex justify-center items-center">
            {form.photo ? (
              ...
              <img
                src={preview}
                alt="preview"
                className="w-9/12 h-9/12 opacity-40"
              />
            )}
